...
public class PedidoService: IPedidoService
{
    ...
    public async Task<ServiceResponse<int>> RegistrarPedido(PedidoCabeceraEntity valores)
    {
        ...
        try
        {
            ...
            if (resultData.Codigo == 0)
            {
                ...
                result.Element = resultData.Ped_Id;
                result.Data = resultData.Ped_Id;
                return result;
            }
    ...
    public async Task<ServiceResponse<int>> RechazarPedido(PedidoCabeceraEntity valores)
    {
        var result = new ServiceResponse<int>();
        try
        {
            var resultData = await _repository.RechazarPedido(valores);
            if (resultData.Codigo == 0)
            {
                result.Success = true;
                result.Message = resultData.Mensaje;
    ...
    public async Task<ServiceResponse<int>> ActualizarReferenciaGeneral(PedidoCabeceraEntity valores)
    {
        var result = new ServiceResponse<int>();
        try
        {
            var resultData = await _repository.ActualizarReferenciaGeneral(valores);
            if (resultData.Codigo == 0)
            {
                result.Success = true;
                result.Message = resultData.Mensaje;
    ...
    public async Task<ServiceResponse<int>> RegistrarArchivoAdjunto(PedidoArchivo valores)
    {
        var result = new ServiceResponse<int>();
        try
        {
            var resultData = await _repository.RegistrarArchivoAdjunto(valores);
            if (resultData.Codigo == 0)
            {
                result.Success = true;
                result.Message = resultData.Mensaje;
    ...
    public async Task<ServiceResponseList<PedidoArchivo>?> ListarArchivosAdjuntos(int? Ped_Cab_Id)
    {
        var result = new ServiceResponseList<PedidoArchivo>();
        try
        {
            var resultData = await _repository.ListarArchivosAdjuntos(Ped_Cab_Id);

            if (resultData == null || !resultData.Any())
            {
                result.Success = true;
                result.Message = "No existe información";
                result.Elements = new List<PedidoArchivo>();
                result.TotalElements = 0;
                return result;
            }
    ...
    public async Task<ServiceResponse<int>> EliminarArchivoAdjunto(PedidoArchivo valores)
    {
        var result = new ServiceResponse<int>();
        try
        {
            var resultData = await _repository.EliminarArchivoAdjunto(valores);
            if (resultData.Codigo == 0)
            {
                result.Success = true;
                result.Message = resultData.Mensaje;
